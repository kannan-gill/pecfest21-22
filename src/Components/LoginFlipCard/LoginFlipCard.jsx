...
const LoginFlipCard = ({ isRegister = false, redirect }) => {
  ...
  const [registerVisible, setRegisterVisible] = useState(isRegister);

  useEffect(() => {
    setTimeout(
      () => {
        setRegisterVisible(toggleFlip);
      },
      500
    )
  }, [toggleFlip]);
  ...
  return (
    <div className={`overflow-hidden vh-100 vw-100 m-0 py-md-4 py-sm-0 d-flex flex-row justify-content-lg-end justify-content-md-center align-items-center`}>
      <Row
        className={`me-lg-5 m-0 h-100 col-lg-4 col-md-6 col-sm-12 col-12 position-relative ${styles.loginCard
