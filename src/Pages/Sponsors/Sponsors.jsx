...
function Sponsors() {
    ...
    getList("sponsors").then((data) => {
      ...
      data.forEach((team) => {
        if (team.sponsors.length > 0) {
          team.sponsors.sort((a, b) =>
            a.rankWithin < b.rankWithin
              ? 1
              : b.rankWithin < a.rankWithin
              ? -1
              : 0
          );
        }
  ...
  return (
    <div style={{ height: "100vh" }} className={`d-flex flex-column ${styles.outerdiv}`}>
      <StarsBg />
      <div
      ...
      >
        <div className="d-flex flex-column align-items-center flex-grow-1 overflow-auto">
          {sponsors.map((sponsor) => {
            return sponsor.sponsors.length > 0 ? (
              <div className="m-4" key={sponsor.id}>
                ...
                <center>
                  <div className={`m-4 w-25 ${styles.headingLine}`}>&nbsp;</div>
                </center>
          ...
          <div className={`m-4 w-25 ${styles.headingLine}`}>&nbsp;</div>
          <div
            style={{ fontFamily: "Audiowide", fontSize: "20px" }}
            className="text-white w-75 mb-4 text-center animate__animated animate__fadeIn"
          >
